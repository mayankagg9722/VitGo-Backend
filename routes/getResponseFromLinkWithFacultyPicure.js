...
function getResponseFromLinkWithFacultyPicure(method, data, link, regNo, password, callback)
{
	...
	const returnMainLinkGet = function(response)
	{
		...
		.jar(cookiejar)
		.strictSSL(false)
		.end(fetchPhotoAndCallback);
	}

	const fetchPhotoAndCallback = function(PageResponse)
	{
		var photoLink = "https://vtop.vit.ac.in/student/emp_photo.asp";
		unirest.get(photoLink)
		.encoding(null)
		.strictSSL(false)
		.end(function(response){
			callback(PageResponse, response.body);
		});
	}
...
module.exports = getResponseFromLinkWithFacultyPicure;